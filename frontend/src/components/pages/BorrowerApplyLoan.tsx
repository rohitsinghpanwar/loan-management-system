...
const hasWarned = useRef(false); // Tracks if warning was already shown
...
const handleAmountChange = (e: ChangeEvent<HTMLInputElement>) => {
  ...
  // Reset warning flag if amount is now valid
  if (!isNaN(totalAmount) && totalAmount >= 10000) {
  ...
  debounceTimeout.current = setTimeout(() => {
    if (!isNaN(totalAmount) && totalAmount < 10000 && !hasWarned.current) {
      toast.warning("Minimum loan amount is 10000");
      hasWarned.current = true; // prevent further warnings until amount is fixed
    }
  }, 500); // 500ms delay
};
...
const prevStep = () => setStep(1);
 const nextDisabled =
    ...
    parseInt(formData.amount || "0", 10) < 10000;
...
const estimatedEMI = months && amount && rate
  ...
  return (
    <div className="flex w-full font-robbert">
